...
function postProcessItem(item, constants) {
    if (item.socketed_items && item.socketed_items.length) {
        // Expose aggregated socketed attributes for UI editors, always an array
        item.socketed_attributes = [];
        for (var _i = 0, _a = item.socketed_items; _i < _a.length; _i++) {
            var socketed = _a[_i];
            // Determine base item group by the parent item type, not the socketed item type
            var pt = constants.armor_items[item.type] || constants.weapon_items[item.type] || constants.other_items[item.type];
            ...
            if (socketed && socketed.magic_attributes) {
...
function describeMods(magic_attributes, constants) {
    ...
    for (var _i = 0, mods_2 = mods; _i < mods_2.length; _i++) {
        ...
        // Compute merge param only for truly parameterized properties, with correct index mapping
        if (prop) {
            switch (prop.dF) {
                case 13: // + to class skills
                    mod.param = mod.values[0];
                    break;
                case 14: // + to skill tab (class only)
                    // pack tab (0..7) and classId into one number: (classId << 3) | tab
                    mod.param = (((_b = mod.values[1]) !== null && _b !== void 0 ? _b : 0) << 3) | (((_c = mod.values[0]) !== null && _c !== void 0 ? _c : 0) & 0x7);
                    break;
                case 16: // aura when equipped
                    mod.param = mod.values[0];
                    break;
                case 24: // charges: param is skill id at index 1
                    mod.param = mod.values[1];
                    break;
                case 27: // + to skill (class only)
                case 28: // + to skill
                    mod.param = mod.values[0];
                    break;
                default:
    ...
    consolidateMods(mods, constants);
    for (var _d = 0, mods_3 = mods; _d < mods_3.length; _d++) {
        var mod = mods_3[_d];
        var prop = constants.magical_properties[mod.id];
...
function describeSingleMod(mod, prop, constants) {
    ...
    switch (prop.dF) {
        ...
        case 13: // +[value] to [class] Skill Levels
            modDesc = formatStr((_b = (_a = constants.classes[mod.values[0]]) === null || _a === void 0 ? void 0 : _a.as) !== null && _b !== void 0 ? _b : prop.dP, val);
            break;
        case 14: // +[value] to [skilltab] Skill Levels ([class] Only)
            var skillTab = (_c = constants.classes[mod.values[1]]) === null || _c === void 0 ? void 0 : _c.ts[mod.values[0]];
            if (skillTab) {
        ...
        case 15: // [chance]% to cast [slvl] [skill] on [event]
            modDesc = modDesc
                ...
                .replace("%s", "" + ((_d = constants.skills[mod.values[1]]) === null || _d === void 0 ? void 0 : _d.n));
            break;
        case 16: // Level [sLvl] [skill] Aura When Equipped
            modDesc = modDesc.replace("%d", "" + val).replace("%s", "" + ((_e = constants.skills[mod.values[0]]) === null || _e === void 0 ? void 0 : _e.n));
            break;
        //todo
        ...
        case 19: //main [value * -1]% [string1]
            if (((_f = mod.values) === null || _f === void 0 ? void 0 : _f.length) >= 2 && mod.values[0] !== mod.values[1]) {
        ...
        case 24: // Level [lvl] [skill] ([curr]/[max] charges)
            modDesc = formatStr(modDesc, mod.values[0], (_g = constants.skills[mod.values[1]]) === null || _g === void 0 ? void 0 : _g.n, mod.values[2], mod.values[3]);
            break;
        case 27: // +[value] to [skill] ([class] Only)
            var skill_1 = constants.skills[mod.values[0]];
            var clazz = (skill_1 === null || skill_1 === void 0 ? void 0 : skill_1.c) != null ? constants.classes.filter(function (e) { return (e === null || e === void 0 ? void 0 : e.c) === (skill_1 === null || skill_1 === void 0 ? void 0 : skill_1.c); })[0] : undefined;
            modDesc = formatStr(modDesc, val, skill_1 === null || skill_1 === void 0 ? void 0 : skill_1.n, clazz === null || clazz === void 0 ? void 0 : clazz.co);
            break;
        case 28: // +[value] to [skill]
            modDesc = formatStr(modDesc, val, (_h = constants.skills[mod.values[0]]) === null || _h === void 0 ? void 0 : _h.n);
            break;
        ...
        case 100:
            ...
            modDesc = modDesc.replace("%d", "" + ((_l = mod.values) === null || _l === void 0 ? void 0 : _l[0])).replace("%d", "" + ((_m = mod.values) === null || _m === void 0 ? void 0 : _m[1]));
            break;
        case 101: // Poison damage
            if (((_o = mod.values) === null || _o === void 0 ? void 0 : _o[0]) === ((_p = mod.values) === null || _p === void 0 ? void 0 : _p[1])) {
                modDesc = modDesc
...
function addModGroups(modifiers, constants) {
    var _a, _b, _c, _d, _e;
    // Normalize duplicates inside each group before generating a group description
    var getParamKey = function (mod) {
        var _a, _b, _c, _d, _e, _f, _g, _h;
        var prop = constants.magical_properties[mod.id];
        switch (prop === null || prop === void 0 ? void 0 : prop.dF) {
            case 13:
            case 16:
            case 27:
            case 28:
                return mod.id + ":" + ((_b = (_a = mod.values) === null || _a === void 0 ? void 0 : _a[0]) !== null && _b !== void 0 ? _b : "x");
            case 14: {
                // same packing used for mod.param: (classId << 3) | tab
                var packed = (((_d = (_c = mod.values) === null || _c === void 0 ? void 0 : _c[1]) !== null && _d !== void 0 ? _d : 0) << 3) | (((_f = (_e = mod.values) === null || _e === void 0 ? void 0 : _e[0]) !== null && _f !== void 0 ? _f : 0) & 0x7);
                return mod.id + ":" + packed;
            }
            case 24:
                return mod.id + ":" + ((_h = (_g = mod.values) === null || _g === void 0 ? void 0 : _g[1]) !== null && _h !== void 0 ? _h : "x"); // skill id for charges
            default:
                return "" + mod.id;
        }
    };
    var _loop_2 = function (group) {
        // Take only group's mods and consolidate duplicates by id+param like in aggregateAttributes
        var inGroup = (_a = modifiers === null || modifiers === void 0 ? void 0 : modifiers.filter(function (_a) {
            var id = _a.id;
            return group.statsInGroup.includes(id);
        })) !== null && _a !== void 0 ? _a : [];
        var map = new Map();
        for (var _i = 0, inGroup_1 = inGroup; _i < inGroup_1.length; _i++) {
            var m = inGroup_1[_i];
            var key = getParamKey(m);
            var existing = map.get(key);
            if (!existing) {
                map.set(key, m);
            }
            else {
                existing.value = ((_b = existing.value) !== null && _b !== void 0 ? _b : 0) + ((_c = m.value) !== null && _c !== void 0 ? _c : 0);
                // keep last value in values[] in sync with summed value
                if (existing.values && existing.values.length) {
                    existing.values[existing.values.length - 1] = existing.value;
                }
            }
        }
        var mods = Array.from(map.values());
        ...
        if (group.isRange && ((_d = mods[0].value) !== null && _d !== void 0 ? _d : 0) > ((_e = mods[1].value) !== null && _e !== void 0 ? _e : 0)) {
            return "continue";
        }
        ...
        if (group.s === "group:secondary-dmg" || group.s === "group:min-dmg" || group.s === "group:max-dmg") {
            ...
            if (modifiers === null || modifiers === void 0 ? void 0 : modifiers.find(function (mod) { return mod.name === "group:primary-dmg"; })) {
                ...
                for (var _a = 0, mods_4 = mods; _a < mods_4.length; _a++) {
                    var mod = mods_4[_a];
        ...
        for (var _b = 0, mods_5 = mods; _b < mods_5.length; _b++) {
            var mod = mods_5[_b];
            delete mod.description;
        }
    };
...
function consolidateMods(mods, constants) {
    var _a, _b;
    var _loop_3 = function (mod) {
        var duplicateIndex = void 0;
        var prop = constants.magical_properties[mod.id];
        var useParamKey = !!(prop && (prop.dF === 13 || prop.dF === 14 || prop.dF === 16 || prop.dF === 24 || prop.dF === 27 || prop.dF === 28));
        while ((duplicateIndex = mods.findIndex(function (other) { return mod !== other && mod.id === other.id && "value" in mod && (useParamKey ? mod.param === other.param : true); })) >= 0) {
...
function allAttributes(item, constants) {
    var socketed_attributes = item.socketed_attributes && item.socketed_attributes.length
        ? JSON.parse(JSON.stringify(item.socketed_attributes))
        : [];
    if (!socketed_attributes.length && item.socketed_items) {
        for (var _i = 0, _a = item.socketed_items; _i < _a.length; _i++) {
            var i = _a[_i];
            if (i && i.magic_attributes) {
