  ...
  <div>
    ...
    <div class="form-row">
      <div class="col-md-2">
        ...
        <a-input-number id="Level" v-model:value="save.attributes.level" :min="min(12)" :max="max(12)" @update:value="() => change(12, save.attributes, 'level')" />
      </div>
      <div class="col-md-2">
        <label for="Experience">Experience</label>
        <a-input-number id="Experience" v-model:value="save.attributes.experience" />
      </div>
    </div>
    <div class="form-row">
      <div class="col-md-4">
        ...
        <div class="input-group">
          <a-input-number id="Life" v-model:value="save.attributes.current_hp" :min="min(6)" :max="max(6)" @update:value="() => change(6, save.attributes, 'current_hp')" />
          <div class="input-group-prepend input-group-append">
            <div class="input-group-text">/</div>
          </div>
          <a-input-number id="MaxLife" v-model:value="save.attributes.max_hp" :min="min(7)" :max="max(7)" @update:value="() => change(7, save.attributes, 'max_hp')" />
        </div>
      </div>
      <div class="col-md-4">
        <label for="Mana">Mana</label>
        <div class="input-group">
          <a-input-number id="Mana" v-model:value="save.attributes.current_mana" :min="min(8)" :max="max(8)" @update:value="() => change(8, save.attributes, 'current_mana')" />
          <div class="input-group-prepend input-group-append">
            <div class="input-group-text">/</div>
          </div>
          <a-input-number id="MaxMana" v-model:value="save.attributes.max_mana" :min="min(9)" :max="max(9)" @update:value="() => change(9, save.attributes, 'max_mana')" />
        </div>
      </div>
      <div class="col-md-4">
        <label for="Stamina">Stamina</label>
        <div class="input-group">
          <a-input-number id="Stamina" v-model:value="save.attributes.current_stamina" :min="min(6)" :max="max(6)" @update:value="() => change(6, save.attributes, 'current_stamina')" />
          <div class="input-group-prepend input-group-append">
            <div class="input-group-text">/</div>
          </div>
          <a-input-number id="MaxStamina" v-model:value="save.attributes.max_stamina" :min="min(7)" :max="max(7)" @update:value="() => change(7, save.attributes, 'max_stamina')" />
        </div>
      </div>
    </div>
    <div class="form-row">
      <div class="col-md-2">
        ...
        <a-input-number id="Strength" v-model:value="save.attributes.strength" :min="min(0)" :max="max(0)" @update:value="() => change(0, save.attributes, 'strength')" />
      </div>
      <div class="col-md-2">
        <label for="Dexterity">Dexterity</label>
        <a-input-number id="Dexterity" v-model:value="save.attributes.dexterity" :min="min(2)" :max="max(2)" @update:value="() => change(2, save.attributes, 'dexterity')" />
      </div>
      <div class="col-md-2">
        <label for="Vitality">Vitality</label>
        <a-input-number id="Vitality" v-model:value="save.attributes.vitality" :min="min(3)" :max="max(3)" @update:value="() => change(3, save.attributes, 'vitality')" />
      </div>
      <div class="col-md-2">
        <label for="Energy">Energy</label>
        <a-input-number id="Energy" v-model:value="save.attributes.energy" :min="min(1)" :max="max(1)" @update:value="() => change(1, save.attributes, 'energy')" />
      </div>
    </div>
    <div class="form-row">
      <div class="col-md-2">
        <label for="UnusedStatPoints">Unused Stat Points</label>
        <a-input-number id="UnusedStatPoints" v-model:value="save.attributes.unused_stats" :min="min(4)" :max="max(4)" @update:value="() => change(4, save.attributes, 'unused_stats')" />
      </div>
      <div class="col-md-2">
        <label for="UnusedSkillPoints">Unused Skill Points</label>
        <a-input-number id="UnusedSkillPoints" v-model:value="save.attributes.unused_skill_points" :min="min(5)" :max="max(5)" @update:value="() => change(5, save.attributes, 'unused_skill_points')" />
      </div>
    </div>
    <div class="form-row">
      <div class="col-md-2">
        <label for="Gold">Gold</label>
        <a-input-number id="Gold" v-model:value="save.attributes.gold" :min="min(14)" :max="max(14)" @update:value="() => change(14, save.attributes, 'gold')" />
      </div>
      <div class="col-md-2">
        <label for="StashedGold">Stashed Gold</label>
        <a-input-number id="StashedGold" v-model:value="save.attributes.stashed_gold" :min="min(15)" :max="max(15)" @update:value="() => change(15, save.attributes, 'stashed_gold')" />
      </div>
    </div>
  </div>
</template>
